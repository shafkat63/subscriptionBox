...
if (isset($_POST['submit'])) {
  if (isset($_GET['token'])) {
    ...
    $query2 = "update users set subscription = '500' where user_id= '$token'";
    ...
    if ($result) {
      if ($result && mysqli_num_rows($result) > 0) {
        ...
        header("Location: product.php?token=$token");
        die;
      }
    }
  }
...
<head>
  <script type="text/javascript">
    function sub() {
      let ans = confirm("Are You sure You want to Subscribe !!");
      if (ans == true) {
        return true;
      } else {
        return false;
      ...
      <form method="post" class="subscribe">
        <div class="subscriptionbox">
          <div class="content">
            ...
            <input class="input" type="submit" name="submit" onclick="return sub();" value="SUBSCRIBE NOW">
          </div>
        </div>
      </form>
